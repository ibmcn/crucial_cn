...
// ****************************************
        ...
        result.delegate('click', '.ddrmax_close', function (s) {
            ...
            KISSY.all('.ddrmax'+ s.target.title).fadeOut();
        });
        ...
        function show_product() {
            ...
            IO.getJSON(product_path, function (data) {
                ...
                if (data == null) {
                    ...
                    S.each(data.products, function (item, i) {
                    ...
                    var ddr_bar='', ssd_bar='', ddr_i = 0, ssd_i = 0,ddr_box='',ssd_box='';
                    S.each(data.attr, function (item, i) {
                        ddr_box += '<div class="ddrmax show ddrmax'+i+'"><span>' + item + '</span><a class="ddrmax_close" title="'+i+'">X</a></div>';
                    });
                    product_list.one('.ddr').prepend('<div class="products"></div>');
                    product_list.one('.ssd').prepend('<div class="products"></div>');
                    for (var i = 0; i < tuples.length; i++) {
                        ...
                        if (item.family === "DDR3" || item.family === "DDR2" || item.family === "DDR") {
                            ...
                            if (item.recommend) {
                                product_list.one('.ddr').one('.products').prepend(option);
                                product_list.one('.ddr').one('.products').prepend('<div class="highlight orange ddrhigh"><div class="htitle">升级推荐</div></div>');
                            }
                            else
                                product_list.one('.ddr').one('.products').append(option);
                            ...
                            */
                            ...
                            if (ssd_i == 0) {
                                ssd_box = '<div class="ssdmax show"><span>M550 更优秀的性能，MX100 更优惠的价格</span><a class="ssdmax_close">X</a></div>';
                                ssd_i += 1;
                            }
                            if (item.recommend) {
                                product_list.one('.ssd').one('.products').prepend(option);
                                product_list.one('.ssd').one('.products').prepend('<div class="highlight orange ssdhigh"><div class="htitle">升级推荐</div></div>');
                            }
                            else
                                product_list.one('.ssd').one('.products').append(option);
                    ...
                    if (ssd_box)
                        product_list.one('.ssd').prepend(ssd_box);
                    if (ddr_box)
                        product_list.one('.ddr').prepend(ddr_box);
